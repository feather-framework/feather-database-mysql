...
openssl req -new -x509 -key ca.key -out ca.pem -days 365 -subj "/CN=MariaDB-CA"
...
# SAN for localhost, mariadb, and 127.0.0.1
echo "subjectAltName=DNS:localhost,DNS:mariadb,IP:127.0.0.1" > san.cnf
...
# convert PKCS#8 private key -> traditional RSA PEM
